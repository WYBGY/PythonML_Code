...
class Layer:
    """
    初始化结构
    self.shape: 记录上一个layer和当前layer的神经元个数：
        shape[0]：上一个layer的神经元数量
        shape[1]: 当前layer的神经元数量
    """
...
class CostLayer(Layer):
    ...
    self._available_transform_functions: 特殊变换函数的字典
    self._cost_function, self._cost_function_name： 损失函数及其名字
    self._transform_function, self._transform_function: 特殊函数及其名字
    """
    ...
    def __init__(self, shape, cost_function='MSE', transform=None):
        ...
        self._available_transform_functions = {'Softmax': CostLayer._softmax,
        ...
        if transform is None and cost_function == 'Crossentropy':
            ...
            self._transform_funtion = CostLayer._softmax
        else:
            self._transform = transform
            self._transform_funtion = self._available_transform_functions.get(transform, None)

    def __str__(self):
        return self._cost_function_name

    def _activate(self, x):
        if self._transform_funtion is None:
            return x
        return self._transform_funtion(x)
    ...
    def bp_first(self, y, y_pred):
        ...
        if self._transform_funtion is None:
            return dy
        return dy * self._transform_funtion(y_pred, diff=True)
